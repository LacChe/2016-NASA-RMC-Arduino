...
public class Structure {
	...
	private static FileNode START;

	public static void setDefault() {
		current = new FileNode("gyrusH");
		START = current;
		current.addFile(new Memory("UnIn",
				new String[] { "Here is Poul Anderson's essay \"Uncleftish Beholding\" (\"Atomic",
						"Theory\"), reprinted from the revised edition appearing in his ",
						"collection _All One Universe_." }));
		current.addFile(new FileNode("gyrusA"));
		FileNode temp = current;
		current = (FileNode) current.getFile("gyrusA");
		current.addFile(new Memory("UnSe",
				new String[] { "The firststuffs have their being as motes called *unclefts*.",
 ...
 * "highly lightrottish and therefore are not found in the", "greenworld.", "",
